...
function vocab(char: string, level: number, uses: string[]): Item {
  return { ...kanji(char, level), id: itemId('vocab', char), type: 'vocab', uses }
}

function radical(char: string, level: number): Item {
  return { ...kanji(char, level), id: itemId('radical', char), type: 'radical' }
}
...
describe('isUnlocked', () => {
  ...
  it('holds a word until every kanji it uses reaches Novice IV (stage 3)', () => {
    ...
    expect(
      isUnlocked(word, { [itemId('kanji', '山')]: p(3), [itemId('kanji', '川')]: p(2) }, 2),
    ).toBe(false)
    expect(
      isUnlocked(word, { [itemId('kanji', '山')]: p(3), [itemId('kanji', '川')]: p(3) }, 2),
    ).toBe(true)
  })

  it('stage 4+ also satisfies the word unlock', () => {
    expect(
      isUnlocked(word, { [itemId('kanji', '山')]: p(4), [itemId('kanji', '川')]: p(7) }, 2),
    ).toBe(true)
  })
})
...
describe('canAdvance', () => {
  ...
  it('ignores radicals and vocab', () => {
    ...
    const map = withAdept(corpus.filter((i) => i.type === 'kanji'), 9)
    expect(canAdvance(1, map, corpus)).toBe(true)
  })
