...
describe('review/submit', () => {
  ...
  it('a correct answer schedules the item and updates stats', () => {
    ...
    expect(s.progress[FIRE]).toMatchObject({
      stage: 1,
      due: T0 + 8 * HOUR_MS,
      seen: 1,
      correct: 1,
...
describe('data/erase', () => {
  it('needs two presses and then wipes progress and custom mnemonics', () => {
    let s: AppState = {
      ...state,
      progress: {
        [FIRE]: { stage: 3, due: T0, lapses: 0, seen: 4, correct: 4 },
      },
      custom: { [FIRE]: 'story' },
      level: 4,
    };
