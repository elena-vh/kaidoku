// One reducer, pure: actions that need the time carry `now` so it never calls
// Date.now(). It may read the static corpus and call engine functions - both
// pure. `session` and `lesson` live here but are never persisted.
...
export function initialState(persisted: PersistedState = defaultState()): AppState {
  return {
    screen: 'today',
    prev: 'today',
...
function buildSession(ids: ItemId[]): SessionState {
  return { queue: [...ids], i: 0, done: 0, right: 0, wrong: 0, total: ids.length, wrongIds: {} }
}
...
export function reducer(state: AppState, action: Action): AppState {
  switch (action.type) {
    ...
    case 'review/submit': {
      ...
      const prev = state.progress[id] ?? { stage: 0 as Stage, due: action.now, lapses: 0, seen: 0, correct: 0 }
      ...
      return {
        ...
        verdict: { kind: outcome, id, from, to: next.stage, sealed: next.stage === 7 },
      }
    }
    ...
    case 'review/advance': {
      ...
      return {
        ...state,
        session: { ...session, queue, i: session.i + 1, done: session.done + 1 },
        verdict: null,
        answer: '',
      }
    }
    ...
    case 'mnemonic/edit': {
      ...
      return { ...state, editing: !state.editing, draft: state.editing ? '' : current }
    }
