...
describe('review loop, keyboard only', () => {
  it('answers a two-card session and lands the ranks correctly', async () => {
    ...
    seedStorage({
      progress: {
        ...
        [WATER]: { stage: 1, due: T0 - HOUR_MS, lapses: 1, seen: 4, correct: 3 },
      },
    })
    render(<App />)
    ...
    const saved = load()
    expect(saved?.progress[FIRE]).toMatchObject({ stage: 3, seen: 6, correct: 6 })
  ...
  it('a near miss retries in place without recording anything', async () => {
    ...
    seedStorage({
      progress: {
        [mountain]: { stage: 3, due: T0 - HOUR_MS, lapses: 0, seen: 6, correct: 6 },
      },
    })
    render(<App />)
    await user.click(screen.getByRole('button', { name: /begin reviews/i }))
    ...
    await user.keyboard('{Enter}')
    const saved = load()
    expect(saved?.progress[mountain]).toMatchObject({ stage: 4, seen: 7, correct: 7 })
  })

  it('close button abandons the session and returns to Today', async () => {
    const user = userEvent.setup()
    seedStorage({
      progress: { [FIRE]: { stage: 0, due: T0 - HOUR_MS, lapses: 0, seen: 1, correct: 1 } },
    })
    render(<App />)
    await user.click(screen.getByRole('button', { name: /begin reviews/i }))
