...
export function LessonScreen() {
  ...
  if (lesson.done) {
    return (
      <section aria-label="Lesson batch entered">
        ...
        <p>
          Each one sits at Novice I and returns in four hours. Drilling them right
          away is the cheapest way to catch the ones that did not stick.
        </p>
        <button type="button" onClick={() => dispatch({ type: 'lessons/drill' })}>
          Drill the batch
        </button>
        <button type="button" onClick={() => dispatch({ type: 'nav', screen: 'today' })}>
          Not now
        </button>
      </section>
    )
  ...
  const readingA =
    item.type === 'vocab' ? item.reading : item.on || '—'
  ...
  const built =
    item.type === 'vocab' ? item.uses.join(' ') : partGlyphs(item).join(' ') || '—'
  ...
  return (
    <section aria-label="Lesson">
      ...
      <h1>{item.meaning}{item.alt.length ? `, ${item.alt.join(', ')}` : ''}</h1>
