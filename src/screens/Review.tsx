...
export function ReviewScreen() {
  ...
  const showReading = state.settings.hints && (item.reading !== '' || item.on !== '')
  ...
  return (
    <section aria-label="Review session">
      <header>
        ...
        />
        <span>
          {Math.min(session.done + 1, session.queue.length)} of {session.queue.length}
        </span>
        <span>{answered > 0 ? `${pct(session.right, answered)} correct` : 'no answers yet'}</span>
      </header>
      ...
      <div>
        ...
        <input
          ...
          type="text"
          value={state.answer}
          onChange={(e) => dispatch({ type: 'review/type', value: e.target.value })}
          onKeyDown={onKeyDown}
          readOnly={locked}
      ...
      <div aria-live="polite" role="status">
        {copy && (
          <div>
            ...
            <button type="button" onClick={() => dispatch({ type: 'review/advance' })}>
              {copy.cta}
            </button>
